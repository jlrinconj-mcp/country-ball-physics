...
- **Cameras:** whole map (the whole arena when it fits; on tall tracks it fills the width and scrolls with the main group instead of shrinking the map to a sliver), follow leader, follow action (the leading pack, leader always in frame; in Last Place Elimination the fight for last place), follow main group. All have smoothing measured in real time; follow cameras have dynamic zoom, and a new round cuts instead of panning.
