...
- **Map registry:** named maps (`src/tracks/maps.ts`): 15 tracks (Plinko, Pinball, Zigzag, Funnel, Spinner, Drop, Marble Run, Hammer Alley, Crushers, Trapdoors, Tumbler, Vortex, Hurdles, Obstacle Mix, and the ultra-long Grand Gauntlet with twelve obstacles for big tournaments, about 40 s a round) and 3 ring arenas (Spinning Ring, Double Ring, Triple Ring). See `tournaments/grand-gauntlet.json`. A map is a fixed module sequence, seeded picks per slot, or a seeded pool. Every mode plays every map (`config.map`, the Map selector, `--map` in the CLI): in a race, on a ring arena, the first to escape wins; in Last Country Standing on a track, reaching the bottom knocks you out and the last one on the track wins; in Elimination Drop the map pours into the boxes; Last Place Elimination picks its map as its scenario. Every map is paced to rounds of about 8–15 s with 32 countries, measured with `npm run maps`.
- **Track editor:** pick, order and remove the modules of a race track; the geometry inside each module stays seeded.
- **Cameras:** leader ↔ last (made for Shorts and Reels: a close-up on the leader, then on whoever is last, taking turns every 3 s with cuts or quick pans and the name on screen; the default in Last Place Elimination), whole map (the whole arena when it fits; on tall tracks it fills the width and scrolls with the main group instead of shrinking the map to a sliver), follow leader, follow action (the leading pack, leader always in frame; in Last Place Elimination the fight for last place), follow main group. All have smoothing measured in real time; follow cameras have dynamic zoom, and a new round cuts instead of panning.
